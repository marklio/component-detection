    ...
    public class RustCrateUtilities
    {
        ...
        public static CargoDependencyData ExtractRootDependencyAndWorkspaceSpecifications(IEnumerable<IComponentStream> cargoTomlComponentStream, ISingleFileComponentRecorder singleFileComponentRecorder)
        {
            ...
            foreach (var cargoTomlFile in cargoTomlComponentStream)
            {
                ...
                {
                    var workspaces = cargoToml.Get<TomlTable>(RustCrateUtilities.WorkspaceKey);

                    var workspaceMembers = workspaces.ContainsKey(RustCrateUtilities.WorkspaceMemberKey) ? workspaces[RustCrateUtilities.WorkspaceMemberKey] : null;
                    var workspaceExclusions = workspaces.ContainsKey(RustCrateUtilities.WorkspaceExcludeKey) ? workspaces[RustCrateUtilities.WorkspaceExcludeKey] : null;
